...
const ServiceOverview = ({ sectionData, onSave, onBrowseLibrary }) => {
  ...
  const [content, setContent] = useState({
    ...
    para1: sectionData?.para1 || "Managing a court reporting agency means balancing client expectations, reporter coordination, billing cycles, and deadlines, all while keeping your firm’s reputation. That’s where TVG Management comes in. We serve as your dependable operational partner, delivering customized administrative support designed specifically for court reporting firms.",
    para2: sectionData?.para2 || "Whether you're scaling your business, dealing with staff shortages, or simply looking to free up time for strategic growth, our experienced team is here to help. We take care of the behind-the-scenes so you can stay focused on delivering exceptional service to your clients.",
    mainImage: sectionData?.image?.url || sectionData?.img || "./services/management-over.png",
  });
                ...
                onClick={async () => {
                  try {
                    ...
                    if (content.mainImageFile && content.mainImageFile instanceof File) {
                      const res = await uploadAPI.uploadImage(content.mainImageFile, 'overview', 'tvg-management');
