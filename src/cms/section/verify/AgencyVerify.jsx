...
const AgencyVerify = ({ sectionData, onSave, onBrowseLibrary }) => {
  const [isEditing, setIsEditing] = useState(false);
  const [showReference, setShowReference] = useState(false);
  const fileInputsRef = useRef({});

  console.log("AgencyVerify sectionData:", sectionData);

  // Map backend sectionData -> local content shape
  const mapSectionToContent = (sd) => ({
    tag: sd?.name || "TVG Verify",
    heading: sd?.title || "End-to-End Employment Screening & Background Checks",
    desc: sd?.para || "Great hires start with great data.",
    ctaText: sd?.cta?.label || "Schedule a call now",
  ...
  React.useEffect(() => {
    console.log('📌 AgencyVerify received sectionData:', sectionData);
                  ...
                  try {
                    ...
                    for (const key of filesToCheck) {
                      ...
                      if (file && file instanceof File) {
                        ...
                        const res = await uploadAPI.uploadImage(file, 'hero', 'tvg-verify');
                    ...
                    console.log('💾 AgencyVerify final payload (after uploads):', payload);
