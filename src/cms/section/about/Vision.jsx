...
const Vision = ({ sectionData, onSave, onBrowseLibrary }) => {
  const [isEditing, setIsEditing] = useState(false);
  const imageRef = useRef(null);

  console.log("📍 Vision sectionData:", sectionData);
  ...
  useEffect(() => {
    const handleImageSelected = (event) => {
      ...
      if (fieldName === "mainImage") {
        console.log("✅ Image selected for Vision image:", imageUrl);
        setContent((prev) => ({
          ...prev,
          image: {
            ...prev.image,
  ...
  const [content, setContent] = useState({
    ...
    para1: sectionData?.para1 || "We strive to be the trusted partner powering the court reporting community with an expert team and scalable support.",
    para2: sectionData?.para2 || "We're building a future where every firm, big or small, has access to proven solutions backed by experience. Through smart tech and passionate people, we're supporting the industry, one partnership at a time.",
  ...
  const handleImageUpdate = async (e) => {
    ...
    try {
      setUploading(true);
      const uploadResult = await uploadAPI.uploadImage(file, 'vision', 'about');
      ...
      if (uploadResult.success) {
        setContent((prev) => ({
          ...
          image: {
            ...
            alt: prev.image?.alt || "Vision image",
          },
        }));
  ...
  return (
    <div className="flex flex-col gap-8 font-manrope">
      ...
      <div className="flex justify-between items-center border-b border-gray-800 pb-4">
        <h2 className="text-xl font-bold text-cyan-400 uppercase tracking-wider">Vision Section</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col gap-2">
          <label className="text-[10px] text-gray-500 font-bold uppercase ml-1">Section Image</label>
          <div
