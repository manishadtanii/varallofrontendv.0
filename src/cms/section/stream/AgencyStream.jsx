...
const AgencyStream = ({ sectionData, onSave, onBrowseLibrary }) => {
  const [isEditing, setIsEditing] = useState(false);
  const [showReference, setShowReference] = useState(false);
  const fileInputsRef = useRef({});

  console.log("AgencyStream sectionData:", sectionData);

  // Map backend sectionData -> local content shape
  const mapSectionToContent = (sd) => ({
    tag: sd?.name || sd?.title || "TVG Stream",
    heading: sd?.title || sd?.name || "Legal Video, Trial Presentation and Conference A/V",
  ...
  React.useEffect(() => {
    console.log('📌 AgencyStream received sectionData:', sectionData);
                  ...
                  try {
                    ...
                    for (const key of filesToCheck) {
                      ...
                      if (file && file instanceof File) {
                        ...
                        const res = await uploadAPI.uploadImage(file, 'hero', 'tvg-stream');
                    ...
                    console.log('💾 AgencyStream final payload (after uploads):', payload);
