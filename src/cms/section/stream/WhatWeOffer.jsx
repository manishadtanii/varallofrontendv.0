...
const WhatWeOffer = ({ sectionData, onSave, onBrowseLibrary }) => {
  ...
  // Map backend items[] array to local state fields for 7 items
  const mapSectionToContent = (sd) => {
    const items = sd?.items || [];
    return {
      heading: sd?.title || "What We Offer",
      c1_title: items[0]?.heading || "Remote Deposition Setup & Hosting",
      c1_desc: items[0]?.description || "We manage platform access, scheduling, permissions, and environment control so you don't have to.",
      c1_img: items[0]?.icon || "./wwo-icon/stream/1.png",

      c2_title: items[1]?.heading || "Participant Training & Onboarding",
      c2_desc: items[1]?.description || "Customized guidance for attorneys, clients, and witnesses to ensure confidence and preparedness before every session.",
      c2_img: items[1]?.icon || "./wwo-icon/stream/2.png",

      c3_title: items[2]?.heading || "Live Technical Support",
      c3_desc: items[2]?.description || "Real-time troubleshooting during depositions and hearings to minimize disruptions and uphold professionalism.",
      c3_img: items[2]?.icon || "./wwo-icon/stream/3.png",

      c4_title: items[3]?.heading || "Secure Exhibit Management",
      c4_desc: items[3]?.description || "Confidential document sharing, digital exhibit marking, and real-time presentation tools to keep proceedings efficient and organized.",
      c4_img: items[3]?.icon || "./wwo-icon/stream/4.png",

      c5_title: items[4]?.heading || "Session Recording & Archiving",
      c5_desc: items[4]?.description || "Comprehensive video and audio capture with optional transcript syncing, security upload, and easily accessible when needed.",
      c5_img: items[4]?.icon || "./wwo-icon/stream/5.png",

      c6_title: items[5]?.heading || "Trial Technician & Hot-Seat Operator Services",
      c6_desc: items[5]?.description || "On-site or remote support for exhibit displays, video playback, and seamless coordination, ensuring flawless execution under pressure.",
      c6_img: items[5]?.icon || "./wwo-icon/stream/6.png",

      c7_title: items[6]?.heading || "Trial Equipment Rentals",
      c7_desc: items[6]?.description || "Access professional-grade courtroom equipment, including screens, projectors, and audio systems, with full setup and on-site support in your rental region at the time of need.",
      c7_img: items[6]?.icon || "./wwo-icon/stream/7.png",
    };
  };
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    const keys = ['c1_img', 'c2_img', 'c3_img', 'c4_img', 'c5_img', 'c6_img', 'c7_img'];
                    for (const k of keys) {
                      const fileKey = `${k}File`;
                      const f = content[fileKey];
                      if (f && f instanceof File) {
                        const res = await uploadAPI.uploadImage(f, 'tvgEffect', 'tvg-stream');
                    ...
                    const payload = {
                      ...
                      items: [
                        ...
                        { heading: content.c5_title, description: content.c5_desc, icon: content.c5_img },
                        { heading: content.c6_title, description: content.c6_desc, icon: content.c6_img },
                        { heading: content.c7_title, description: content.c7_desc, icon: content.c7_img },
                      ]
                    };
