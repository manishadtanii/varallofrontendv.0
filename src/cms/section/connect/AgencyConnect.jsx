...
const AgencyConnect = ({ sectionData, onSave, onBrowseLibrary }) => {
  const [isEditing, setIsEditing] = useState(false);
  const [showReference, setShowReference] = useState(false);
  const fileInputsRef = useRef({});

  console.log("AgencyConnect sectionData:", sectionData);

  // Map backend sectionData -> local content shape
  const mapSectionToContent = (sd) => ({
    tag: sd?.name || sd?.title || "TVG Connect",
    heading: sd?.title || sd?.name || "Association Management for the Court Reporting Industry",
    desc: sd?.para || sd?.description || "Professional organizations and trade associations thrive on connection, coordination, and clear leadership. But managing all the moving parts can be overwhelming.",
  ...
  React.useEffect(() => {
    console.log('📌 AgencyConnect received sectionData:', sectionData);
                  ...
                  try {
                    ...
                    for (const key of filesToCheck) {
                      ...
                      if (file && file instanceof File) {
                        ...
                        const res = await uploadAPI.uploadImage(file, 'hero', 'tvg-connect');
                    ...
                    console.log('💾 AgencyConnect final payload (after uploads):', payload);
              ...
              <div key={num} className="relative">
                <div className="flex items-center justify-between">
                  ...
                  {isEditing && onBrowseLibrary && (
                    <button
                      onClick={() => onBrowseLibrary(imageKey)}
                ...
                <div
                  onClick={() => isEditing && fileInputsRef.current[imageKey]?.click()}
                ...
                >
                  <input
                    type="file"
                    ref={(el) => (fileInputsRef.current[imageKey] = el)}
                    onChange={(e) => handleImageUpdate(imageKey, e)}
                    className="hidden"
                  />
                  <img
                    src={showReference ? "./hero.png" : content[imageKey]}
