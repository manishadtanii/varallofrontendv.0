...
const ConnectServiceOverview = ({ sectionData, onSave, onBrowseLibrary }) => {
  const [isEditing, setIsEditing] = useState(false);
  const [showReference, setShowReference] = useState(false);
  const fileInputRef = useRef(null);

  console.log("ConnectServiceOverview sectionData:", sectionData);
  console.log(sectionData.para1)
  console.log(sectionData.para2)

  const mapSectionToContent = (sd) => {
    let para1 = "",
      para2 = "";

    // First priority: direct para1/para2 fields
    if (sd?.para1 || sd?.para2) {
      para1 = sd?.para1 || "";
      para2 = sd?.para2 || "";
    }
    // Fallback: para array format
    else if (sd?.para && Array.isArray(sd.para)) {
      para1 = sd.para[0] || "";
      para2 = sd.para[1] || "";
    }
    // Last resort: string para
    else if (sd?.para && typeof sd.para === 'string') {
      para1 = sd.para;
    }

    return {
      heading: sd?.heading || "Service Overview",
      subHeading: sd?.subHeading || sd?.title || "Stronger Communities. Smarter Management.",
      para1,
      para2,
  ...
  React.useEffect(() => {
    console.log('📌 ConnectServiceOverview received sectionData:', sectionData);
                ...
                onClick={async () => {
                  try {
                    ...
                    if (content.imageFile && content.imageFile instanceof File) {
                      ...
                      const res = await uploadAPI.uploadImage(content.imageFile, 'overview', 'tvg-connect');
                    ...
                    console.log('💾 ConnectServiceOverview final payload:', payload);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              className={`w-full bg-transparent border rounded-xl px-4 py-2 outline-none transition-all ${isEditing ? "border-cyan-400" : "border-gray-800 text-gray-400"
                }`}
            />
          </div>
          <div className="flex flex-col gap-1">
        ...
        <div className="flex flex-col gap-2">
          ...
          <div
            ...
            className={`relative border-2 border-dashed rounded-2xl h-96 flex items-center justify-center bg-black/20 overflow-hidden ${isEditing
          ...
          <div className="bg-gray-900/50 rounded-xl p-6 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <h2 className="text-2xl font-bold text-white mb-3">{content.subHeading}</h2>
...
export default ConnectServiceOverview;
