...
const BooksServiceOverview = ({ sectionData, onSave, onBrowseLibrary }) => {
  ...
  const handleCancel = () => {
    setContent({
      heading: sectionData?.heading || "Service Overview",
      subHeading: sectionData?.title || "Precision You Can Count On",
      para1: sectionData?.para?.[0] || sectionData?.para1 || "Running a court reporting firm means juggling many balls, and finances shouldn't be the one that keeps you up at night.",
      para2: sectionData?.para?.[1] || sectionData?.para2 || "Whether you're managing monthly invoices or prepping for year-end filing, we ensure your numbers stay clean, organized, and in control.",
      mainImage: sectionData?.image?.url || sectionData?.img || "./services/books-over.png",
    });
    setIsEditing(false);
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      if (fieldName === 'mainImage') {
        console.log(`🖼️ BooksServiceOverview image selected for ${fieldName}:`, imageUrl);
                ...
                onClick={async () => {
                  try {
                    ...
                    if (content.mainImageFile && content.mainImageFile instanceof File) {
                      const res = await uploadAPI.uploadImage(content.mainImageFile, 'overview', 'tvg-books');
                    ...
                    const payload = {
                      ...
                      para: [content.para1, content.para2],
                      img: content.mainImage,
                    };
                    console.log('💾 BooksServiceOverview final payload:', payload);

                    await onSave(payload);
                    setIsEditing(false);
                  } catch (err) {
                    console.error('❌ Error uploading or saving BooksServiceOverview:', err);
...
export default BooksServiceOverview;
